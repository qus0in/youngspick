    ...
    drawStoreList: function(){
        ...
        for (let v of this.getAllKeys()) {
            ...
            const storeObj = {
                ...
                review: storeVal[2],
                rate: storeVal[3],
                timeStamp: storeVal[4]
            }
            // store 객체 바탕으로 card 생성
            const storeCard = bEl('div', ['card', 'w-100', 'm-3']);
            // card-header
            storeCard.appendChild(bEl('header', ['card-header'], `${storeObj.name} (${storeObj.address})`))
            // card-body
            ...
            // 리뷰 글
            storeCardBody.appendChild(bEl('div', ['card-text'], storeObj.review));
            storeCard.appendChild(storeCardBody);
            // card-footer
            storeCard.appendChild(bEl('footer', ['card-footer', 'text-right'], storeObj.timeStamp));
...
const StoreData = {
    ...
    addStore: function() {
        ...
        allVal = (...ids) => ids.reduce((acc, cur) => `${acc};${Common.getEl(cur, 'v')}`, "");
        // setItem(k,v)으로 저장, getItem(k)로 불러오기
        const ts = this.timeStamp();
        const v = `${allVal('inputStoreName', 'inputStoreAddress', 'inputStoreReview', 'inputStoreRate')};${ts}`.slice(1);
        // console.log(v);
